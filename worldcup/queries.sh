...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON teams.team_id = games.winner_id WHERE year=2018
AND round='Final'")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games LEFT JOIN teams ON teams.team_id = games.winner_id ORDER
BY name")"
